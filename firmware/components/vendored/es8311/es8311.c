 ...
 *
 * Ported from the Waveshare Arduino demo to ESP-IDF's new I2C master driver.
 * Logic / register-level sequencing preserved verbatim; only the I2C access
 * path and the create/delete plumbing were rewritten.
 */
...
#define ES8311_I2C_SCL_HZ        (400000)
...
{
    if (!bus) {
        ESP_LOGE(TAG, "es8311_create: bus handle is NULL");
        return NULL;
    }
    ...
    esp_err_t err = i2c_master_bus_add_device(bus, &dev_cfg, &sensor->i2c_dev);
    if (err != ESP_OK) {
